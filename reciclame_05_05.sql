...
COMMIT;
ALTER TABLE notificaciones MODIFY COLUMN tipo VARCHAR(50);
DELETE FROM notificaciones WHERE id_notificacion IN (
    SELECT id FROM (
        SELECT id_notificacion as id FROM notificaciones 
        WHERE tipo NOT IN (
            'ADMIN_CREADO', 'ADMIN_BLOQUEADO', 'ADMIN_ELIMINADO',
            'ADMIN_EDITADO', 'DOMINIO_CREADO', 'DOMINIO_DESACTIVADO',
            'DOMINIO_ELIMINADO', 'SISTEMA_ALERTA'
        )
    ) AS tmp
);
