-- ============================================================
-- SCRIPT UNIFICADO - REPOSITORIO CIRCULAR RECÍCLAME
-- Motor:   MySQL 8.0+
-- Stack:   Spring Data JPA + MySQL
--
-- Orden de aplicación:
--   1. reciclame_30_05.sql         → Esquema base + datos iniciales
--   2. migration_nuevas_funciones  → contenido_guardado, historial_lectura
--   3. migration_carpetas          → espacio_carpeta, FK en contenido_guardado
--   4. migration_preview_pdf       → archivo_preview_url en estudios
--
-- Ejecutar completo en una sola pasada. No ejecutar los archivos
-- individuales si ya se corrió este script.
-- ============================================================


-- ============================================================
-- PARTE 1: ESQUEMA BASE + DATOS INICIALES (reciclame_30_05.sql)
-- ============================================================
-- SCRIPT DDL + SEED FINAL v3.0 - REPOSITORIO CIRCULAR RECÍCLAME
-- Motor:   MySQL 8.0+
-- Stack:   Spring Data JPA + MySQL
-- ============================================================
-- CAMBIOS RESPECTO A v2.2
-- 1. Se elimina usuarios.dni
-- 2. Se elimina usuario_empresa.ruc
-- 3. Se elimina solicitud_registro.dni y solicitud_registro.ruc
-- 4. Nueva tabla identificacion con tipo ENUM('DNI','RUC') y numero
--    - Socios: pueden registrarse con DNI o RUC
...
-- ============================================================
-- TABLA 2: usuarios
-- dni eliminado → ver tabla identificacion
-- ============================================================
...
-- ruc eliminado → ver tabla identificacion
-- ============================================================
...
-- ============================================================
-- TABLA 4: identificacion
-- Centraliza DNI o RUC de cada usuario.
-- Socios: pueden usar DNI (8 dígitos) o RUC (11 dígitos).
-- Visualizadores: solo DNI.
-- Un mismo RUC puede ser compartido por varios socios de
-- la misma empresa (no se aplica UNIQUE sobre numero+tipo).
-- ============================================================
CREATE TABLE identificacion (
    id_identificacion BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    id_usuario        BIGINT UNSIGNED NOT NULL,
    tipo              ENUM('DNI','RUC') NOT NULL,
    numero            VARCHAR(11)      NOT NULL,

    PRIMARY KEY (id_identificacion),
    UNIQUE KEY uq_identificacion_usuario (id_usuario),
    INDEX idx_identificacion_numero_tipo (numero, tipo),

    CONSTRAINT fk_identificacion_usuario
        FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_unicode_ci
COMMENT='Identificacion de cada usuario (DNI o RUC). Socios: DNI o RUC. Visualizadores: solo DNI.';

-- ============================================================
-- TABLA 5: solicitud_registro
-- dni y ruc reemplazados por tipo_identificacion y numero_identificacion
-- ============================================================
CREATE TABLE solicitud_registro (
    ...
    apellido_paterno      VARCHAR(100)    NULL,
    apellido_materno      VARCHAR(100)    NULL,
    tipo_identificacion   ENUM('DNI','RUC') NOT NULL,
...
-- ============================================================
-- TABLA 7: recuperacion_contrasena
-- ============================================================
...
CREATE TABLE registro_sesiones (
    ...
    rol_nombre        VARCHAR(50)     NULL COMMENT 'Snapshot del rol al momento del registro',
...
CREATE TABLE estudios (
    ...
    anio                YEAR            NOT NULL,
    formato             ENUM('PDF','PPTX','XLSX') NOT NULL,
...
-- ============================================================
-- TABLA 20: auditoria_acciones_admin
-- ============================================================
...
-- ============================================================
-- TABLA 26: configuracion_chatbot
-- ============================================================
...
-- ------------------------------------------------------------
-- USUARIOS (sin columna dni)
-- ------------------------------------------------------------
INSERT INTO usuarios
(id_usuario, id_rol, nombres, apellido_paterno, apellido_materno, correo, telefono, contrasena_hash, url_avatar,
 estado_aprobacion, estado_cuenta, ultimo_acceso, fecha_registro, actualizado_en, eliminado_en)
VALUES
(1,  1, 'Adrian',   'Moreno',   'Soto',   'adrian.moreno@reciclame.pe',      '999111111', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-15 09:05:00', '2026-03-25 08:00:00', '2026-04-15 09:05:00', NULL),
(2,  2, 'Matthew',  'Salvador', 'Sotelo', 'a20222079@pucp.edu.pe',           '999222222', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-15 08:40:00', '2026-03-26 09:30:00', '2026-04-15 08:40:00', NULL),
(3,  2, 'Mariana',  'Huertas',  'Oliva',  'mariana.huertas@reciclame.pe',    '999333333', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-14 18:22:00', '2026-03-27 10:15:00', '2026-04-14 18:22:00', NULL),
(4,  3, 'Marco',    'Silva',    'Perez',  'marco.silva@alicorp.com',         '999444444', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-15 07:55:00', '2026-03-29 11:00:00', '2026-04-15 07:55:00', NULL),
(5,  3, 'Paola',    'Flores',   'Mendez', 'paola.flores@cicla.pe',           '999555555', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-14 20:10:00', '2026-03-30 14:20:00', '2026-04-14 20:10:00', NULL),
(6,  3, 'Roberto',  'Bravo',    'Castro', 'roberto.bravo@greenloop.pe',      '999666666', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'BLOQUEADO', '2026-04-10 16:10:00', '2026-03-31 09:45:00', '2026-04-10 16:10:00', NULL),
(7,  4, 'Elena',    'Torres',   'Ruiz',   'elena.torres@upn.edu.pe',         '999777777', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-15 09:11:00', '2026-04-01 08:50:00', '2026-04-15 09:11:00', NULL),
(8,  4, 'Diego',    'Mendoza',  'Salas',  'diego.mendoza@pucp.edu.pe',       '999888888', '$2a$10$HD..tOqvIDKnRxRTPamcrOtp.IUyB8Dx9ys/jaZ2LY79l2O.Qk/A.', NULL, 'APROBADO', 'ACTIVO',    '2026-04-13 17:00:00', '2026-04-02 17:10:00', '2026-04-13 17:00:00', NULL),
(9,  3, 'Rosa',     'Vega',     'Rios',   'rosa.vega@ecoperu.pe',            '999123123', '$2a$10$pendinghashdemo9',                                         NULL, 'PENDIENTE', NULL,       NULL,                  '2026-04-05 12:00:00', '2026-04-05 12:00:00', NULL),
(10, 4, 'Jorge',    'Paredes',  'Campos', 'jorge.paredes@viewer.pe',         '999234234', '$2a$10$rechashdemo10',                                            NULL, 'RECHAZADO', NULL,       NULL,                  '2026-04-03 16:30:00', '2026-04-06 09:00:00', NULL),
(11, 3, 'Andrea',   'Vargas',   'Peña',   'andrea.vargas@alicorp.com',       '988111111', '$2a$10$hash11',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 08:10:00', '2026-04-06 08:00:00', '2026-04-16 08:10:00', NULL),
(12, 3, 'Miguel',   'Cruz',     'Luna',   'miguel.cruz@greenloop.pe',        '988111112', '$2a$10$hash12',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 09:10:00', '2026-04-06 09:00:00', '2026-04-16 09:10:00', NULL),
(13, 4, 'Sofia',    'Huaman',   'Rojas',  'sofia.huaman@upn.edu.pe',         '988111113', '$2a$10$hash13',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 10:10:00', '2026-04-07 09:00:00', '2026-04-16 10:10:00', NULL),
(14, 4, 'Pedro',    'Navarro',  'Leon',   'pedro.navarro@pucp.edu.pe',       '988111114', '$2a$10$hash14',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 10:40:00', '2026-04-07 11:00:00', '2026-04-16 10:40:00', NULL),
(15, 3, 'Valeria',  'Suarez',   'Gil',    'valeria.suarez@cicla.pe',         '988111115', '$2a$10$hash15',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 11:15:00', '2026-04-08 08:00:00', '2026-04-16 11:15:00', NULL),
(16, 4, 'Luis',     'Quispe',   'Mora',   'luis.quispe@upn.edu.pe',          '988111116', '$2a$10$hash16',                                                   NULL, 'APROBADO', 'ACTIVO',    '2026-04-16 12:05:00', '2026-04-08 10:00:00', '2026-04-16 12:05:00', NULL),
(17, 3, 'Camila',   'Rojas',    'Tapia',  'camila.rojas@ecoperu.pe',         '988111117', '$2a$10$hash17',                                                   NULL, 'APROBADO', NULL,       NULL,                  '2026-04-09 12:00:00', '2026-04-09 12:00:00', NULL),
(18, 4, 'Alvaro',   'Molina',   'Sosa',   'alvaro.molina@viewer.pe',         '988111118', '$2a$10$hash18',                                                   NULL, 'RECHAZADO', NULL,       NULL,                  '2026-04-10 15:00:00', '2026-04-10 15:00:00', NULL);

-- ------------------------------------------------------------
-- IDENTIFICACION
-- Socios con empresa → RUC | Socios sin empresa → DNI
-- Visualizadores → DNI | Admins → DNI
-- ------------------------------------------------------------
INSERT INTO identificacion (id_usuario, tipo, numero) VALUES
(1,  'DNI', '72643574'),
(2,  'DNI', '20222079'),
(3,  'DNI', '74567890'),
(4,  'RUC', '20100070970'),
...
-- ------------------------------------------------------------
-- USUARIO_EMPRESA (sin columna ruc)
-- ------------------------------------------------------------
INSERT INTO usuario_empresa
(id_usuario_empresa, id_usuario, razon_social, cargo)
VALUES
(1, 4,  'Alicorp S.A.A.',            'Coordinador de sostenibilidad'),
(2, 5,  'Cicla Perú S.A.C.',         'Analista de economía circular'),
(3, 6,  'GreenLoop Consultores',     'Jefe de operaciones'),
(4, 9,  'EcoPerú Innovación S.A.C.', 'Especialista ambiental'),
(5, 11, 'Alicorp S.A.A.',            'Analista de proyectos'),
(6, 12, 'GreenLoop Consultores',     'Consultor senior'),
(7, 15, 'Cicla Perú S.A.C.',         'Especialista de cumplimiento'),
(8, 17, 'EcoPerú Innovación S.A.C.', 'Postulante');

-- ------------------------------------------------------------
-- SOLICITUD_REGISTRO (tipo_identificacion + numero_identificacion)
-- ------------------------------------------------------------
INSERT INTO solicitud_registro
(id_solicitud, nombres, apellido_paterno, apellido_materno, tipo_identificacion, numero_identificacion,
 correo, telefono, rol_solicitado, estado, motivo_rechazo, revisado_por, id_usuario_creado, fecha_solicitud, fecha_resolucion)
VALUES
(1,  'Marco',   'Silva',   'Perez',  'RUC', '20100070970', 'marco.silva@alicorp.com',   '999444444', 'SOCIO',        'APROBADO',  NULL, 2, 4,  '2026-03-28 10:10:00', '2026-03-29 10:00:00'),
(2,  'Paola',   'Flores',  'Mendez', 'RUC', '20604561234', 'paola.flores@cicla.pe',     '999555555', 'SOCIO',        'APROBADO',  NULL, 3, 5,  '2026-03-29 11:30:00', '2026-03-30 14:00:00'),
(3,  'Elena',   'Torres',  'Ruiz',   'DNI', '78901234',    'elena.torres@upn.edu.pe',   '999777777', 'VISUALIZADOR', 'APROBADO',  NULL, 2, 7,  '2026-03-31 08:30:00', '2026-04-01 08:30:00'),
(4,  'Jorge',   'Paredes', 'Campos', 'DNI', '71234567',    'jorge.paredes@viewer.pe',   '999234234', 'VISUALIZADOR', 'RECHAZADO', 'El dominio de correo no está autorizado.', 3, NULL, '2026-04-03 15:00:00', '2026-04-06 09:00:00'),
(5,  'Rosa',    'Vega',    'Rios',   'RUC', '20599887766', 'rosa.vega@ecoperu.pe',      '999123123', 'SOCIO',        'PENDIENTE', NULL, NULL, NULL, '2026-04-05 12:00:00', NULL),
(6,  'Andrea',  'Vargas',  'Peña',   'RUC', '20100070970', 'andrea.vargas@alicorp.com', '988111111', 'SOCIO',        'APROBADO',  NULL, 2, 11, '2026-04-06 08:00:00', '2026-04-06 10:00:00'),
(7,  'Miguel',  'Cruz',    'Luna',   'RUC', '20607894561', 'miguel.cruz@greenloop.pe',  '988111112', 'SOCIO',        'APROBADO',  NULL, 3, 12, '2026-04-06 09:00:00', '2026-04-06 11:00:00'),
(8,  'Sofia',   'Huaman',  'Rojas',  'DNI', '72345673',    'sofia.huaman@upn.edu.pe',   '988111113', 'VISUALIZADOR', 'APROBADO',  NULL, 2, 13, '2026-04-07 09:00:00', '2026-04-07 12:00:00'),
(9,  'Pedro',   'Navarro', 'Leon',   'DNI', '72345674',    'pedro.navarro@pucp.edu.pe', '988111114', 'VISUALIZADOR', 'APROBADO',  NULL, 2, 14, '2026-04-07 11:00:00', '2026-04-07 13:00:00'),
(10, 'Valeria', 'Suarez',  'Huertas','RUC', '20604561234', 'valeria.suarez@cicla.pe',   '988111115', 'SOCIO',        'APROBADO',  NULL, 3, 15, '2026-04-08 08:00:00', '2026-04-08 10:00:00');
...
(id_sesion, id_usuario, token, fecha_inicio, fecha_fin, duracion_minutos, ip, agente_usuario, estado, rol_nombre)
VALUES
(1,  1,  'jwt-superadmin-001', '2026-04-15 08:50:00', NULL,                  NULL, '190.12.20.1',  'Chrome/Windows',  'VIGENTE',    'SUPERADMIN'),
(2,  2,  'jwt-admin-001',      '2026-04-15 08:15:00', '2026-04-15 09:00:00', 45,   '190.12.20.2',  'Chrome/Mac',      'FINALIZADA', 'ADMIN'),
(3,  4,  'jwt-socio-001',      '2026-04-15 07:55:00', NULL,                  NULL, '190.12.20.3',  'Edge/Windows',    'VIGENTE',    'SOCIO'),
(4,  5,  'jwt-socio-002',      '2026-04-14 20:10:00', '2026-04-14 21:05:00', 55,   '190.12.20.4',  'Chrome/Linux',    'FINALIZADA', 'SOCIO'),
(5,  7,  'jwt-view-001',       '2026-04-15 09:11:00', NULL,                  NULL, '190.12.20.5',  'Firefox/Windows', 'VIGENTE',    'VISUALIZADOR'),
(6,  8,  'jwt-view-002',       '2026-04-13 16:15:00', '2026-04-13 17:00:00', 45,   '190.12.20.6',  'Safari/iPad',     'EXPIRADA',   'VISUALIZADOR'),
(7,  11, 'jwt-socio-003',      '2026-04-16 08:10:00', NULL,                  NULL, '190.12.20.7',  'Chrome/Windows',  'VIGENTE',    'SOCIO'),
(8,  12, 'jwt-socio-004',      '2026-04-16 09:10:00', NULL,                  NULL, '190.12.20.8',  'Chrome/Linux',    'VIGENTE',    'SOCIO'),
(9,  13, 'jwt-view-003',       '2026-04-16 10:10:00', NULL,                  NULL, '190.12.20.9',  'Firefox/Linux',   'VIGENTE',    'VISUALIZADOR'),
(10, 14, 'jwt-view-004',       '2026-04-16 10:40:00', NULL,                  NULL, '190.12.20.10', 'Edge/Windows',    'VIGENTE',    'VISUALIZADOR');
...
(2,  'Diagnóstico de trazabilidad para cadenas de reciclaje',
     ...
     '1779122765118_diagnostico_trazabilidad_2025.pptx', 'estudios/1779122765118_diagnostico_trazabilidad_2025.pptx', 89,
     '2025-02-20', 88, 'Versión de consulta para socios y visualizadores.', 3, 3, '2026-03-30 09:30:00', '2026-05-18 11:46:05', NULL),
(3,  'Estudio de mercado para subproductos orgánicos',
     ...
     '1779122740336_mercado_subproductos_organicos_2023.pdf', 'estudios/1779122740336_mercado_subproductos_organicos_2023.pdf', 4,
     NULL, 75, 'Borrador interno en revisión legal.', 2, NULL, '2026-04-01 10:15:00', '2026-05-18 11:45:40', NULL),
(4,  'Brechas regulatorias para economía circular en envases',
     ...
     '1779122723992_brechas_regulatorias_envases_2025.pdf', 'estudios/1779122723992_brechas_regulatorias_envases_2025.pdf', 4,
     '2025-03-18', 95, 'Análisis con fines de incidencia y apoyo regulatorio.', 2, 2, '2026-04-02 08:40:00', '2026-05-18 11:45:24', NULL),
(5,  'Hoja técnica de indicadores de circularidad empresarial',
     ...
     '1779122699923_indicadores_circularidad_2022.pptx', 'estudios/1779122699923_indicadores_circularidad_2022.pptx', 89,
     '2022-11-22', 60, 'Sustituido por una versión más reciente.', 3, 2, '2026-04-03 15:10:00', '2026-05-18 11:45:00', NULL),
(6,  'Potencial de valorización de vidrio posconsumo',
     ...
     '1779122686848_valorizacion_vidrio_2024.pdf', 'estudios/1779122686848_valorizacion_vidrio_2024.pdf', 4,
     '2024-09-10', 84, 'Uso para proyectos y diagnósticos sectoriales.', 2, 3, '2026-04-05 10:00:00', '2026-05-18 11:44:47', NULL),
(7,  'Diagnóstico de capacidades para logística inversa',
     ...
     '1779122659735_logistica_inversa_2025.pdf', 'estudios/1779122659735_logistica_inversa_2025.pdf', 4,
     '2025-01-15', 81, 'Documento de consulta general.', 3, 2, '2026-04-06 09:00:00', '2026-05-18 11:44:20', NULL),
(8,  'Tendencias de innovación en reciclaje químico',
     ...
     '1779122632554_innovacion_reciclaje_quimico_2026.pptx', 'estudios/1779122632554_innovacion_reciclaje_quimico_2026.pptx', 90,
     NULL, 77, 'Borrador para validación técnica.', 2, NULL, '2026-04-07 11:00:00', '2026-05-18 11:43:53', NULL),
(9,  'Benchmark regional de indicadores circulares',
     ...
     '1779122606148_benchmark_indicadores_2024.pdf', 'estudios/1779122606148_benchmark_indicadores_2024.pdf', 5,
     '2024-12-02', 90, 'Documento de apoyo para política pública.', 3, 3, '2026-04-08 10:00:00', '2026-05-18 11:43:26', NULL),
(10, 'Guía rápida de trazabilidad documental',
     ...
     '1779122583782_guia_trazabilidad_documental_2026.pdf', 'estudios/1779122583782_guia_trazabilidad_documental_2026.pdf', 4,
     '2026-04-01', 70, 'Consulta general para usuarios internos.', 2, 2, '2026-04-08 11:00:00', '2026-05-18 11:58:34', NULL);

-- ------------------------------------------------------------
-- ESTUDIO_CATEGORIA
-- ------------------------------------------------------------
INSERT INTO estudio_categoria (id_estudio_categoria, id_estudio, id_categoria) VALUES
(1, 1, 6), (2, 1, 7),
(3, 2, 8), (4, 2, 9),
(5, 3, 6), (6, 3, 10),
(7, 4, 7), (8, 4, 10),
...
(2,  'Reglamento técnico sobre responsabilidad extendida del productor',
     ...
     '1779122195474_reglamento_responsabilidad_extendida_productor_2025.pdf', 'normativas/1779122195474_reglamento_responsabilidad_extendida_productor_2025.pdf', 'https://mma.gob.cl/ministerio-del-medio-ambiente-inicia-proceso-de-consulta-publica-del-anteproyecto-del-reglamento-de-la-ley-de-plasticos-de-un-solo-uso/',
     3, 3, '2026-03-30 10:00:00', '2026-05-18 11:36:36', NULL),
...
(4,  'Hoja de ruta para infraestructura de valorización regional',
     ...
     '1779122250786_hoja_ruta_infraestructura_valorizacion_regional_2026.pdf', 'normativas/1779122250786_hoja_ruta_infraestructura_valorizacion_regional_2026.pdf', 'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=COM%3A2020%3A98%3AFIN',
     3, 2, '2026-04-02 13:15:00', '2026-05-18 11:37:31', NULL),
(5,  'Anteproyecto de decreto sobre trazabilidad digital de residuos',
     ...
     '1779122300103_anteproyecto_trazabilidad_digital_residuos_2026.pdf', 'normativas/1779122300103_anteproyecto_trazabilidad_digital_residuos_2026.pdf', 'https://sinia.minam.gob.pe/normas',
     2, 2, '2026-04-04 10:20:00', '2026-05-18 11:38:20', NULL),
(6,  'Decreto sobre eficiencia energética industrial',
     ...
     '1779122335210_decreto_eficiencia_energetica_industrial_2025.pdf', 'normativas/1779122335210_decreto_eficiencia_energetica_industrial_2025.pdf', 'https://www.gob.pe/institucion/minem/normas-legales/395164-004-2016-em',
     2, 2, '2026-04-05 10:00:00', '2026-05-18 11:38:55', NULL),
(7,  'Guía internacional de simbiosis industrial',
     ...
     NULL, NULL, 'https://link.springer.com/article/10.1111/jiec.13478',
     3, NULL, '2026-04-05 11:00:00', '2026-05-18 11:39:31', NULL),
(8,  'Reglamento para valorización de RAEE',
     ...
     '1779122427391_reglamento_valorizacion_raee_2025.pdf', 'normativas/1779122427391_reglamento_valorizacion_raee_2025.pdf', 'https://www.gob.pe/institucion/minam/informes-publicaciones/1503643-regimen-especial-de-gestion-y-manejo-de-residuos-de-aparatos-electricos-y-electronicos',
     2, 3, '2026-04-06 08:00:00', '2026-05-18 11:40:27', NULL),
(9,  'Ley comparada sobre envases reutilizables',
     ...
     NULL, NULL, 'https://www.oecd-ilibrary.org/en/topics/resource-efficiency-and-circular-economy.html',
     2, NULL, '2026-04-06 09:00:00', '2026-05-18 11:41:26', NULL),
(10, 'Lineamientos para compras públicas circulares',
     ...
     '1779122523625_lineamientos_compras_publicas_circulares_2026.pdf', 'normativas/1779122523625_lineamientos_compras_publicas_circulares_2026.pdf', 'https://www.elperuano.pe/noticia/286783-nuevas-reglas-del-oece-revisa-los-lineamientos-obligatorios-para-compradores-publicos',
     3, 2, '2026-04-07 10:00:00', '2026-05-18 11:42:04', NULL);

-- ------------------------------------------------------------
-- NORMATIVA_CATEGORIA
-- ------------------------------------------------------------
INSERT INTO normativa_categoria (id_normativa_categoria, id_normativa, id_categoria) VALUES
(1, 1, 2),  (2, 1, 1),
(3, 2, 4),  (4, 2, 1),
(5, 3, 1),  (6, 3, 5),
(7, 4, 2),
...
-- INDICADOR
-- ------------------------------------------------------------
INSERT INTO indicador (id_indicador, nombre, descripcion, categoria, unidad, estado) VALUES
...
-- INDICADOR_VALOR
-- ------------------------------------------------------------
INSERT INTO indicador_valor (id_valor, id_indicador, valor, anio, fecha_registro) VALUES
...
(2, 'Editorial: trazabilidad como base de la confianza',
 ...
 'EDITORIAL', NULL, NULL, '2026-04-14', 1, '2026-04-14 07:40:00', NULL),
...
-- ACTIVIDAD_SISTEMA
-- ------------------------------------------------------------
INSERT INTO actividad_sistema (id_actividad, tipo, id_referencia, descripcion, accion, fecha) VALUES
...
-- CHAT_SESION
-- ------------------------------------------------------------
INSERT INTO chat_sesion (id_chat, id_usuario, fecha_inicio, fecha_fin, activa) VALUES
...
-- CHAT_MENSAJE
-- ------------------------------------------------------------
INSERT INTO chat_mensaje (id_mensaje, id_chat, emisor, mensaje, fecha) VALUES
...
-- ============================================================
-- PARTE 2: NUEVAS FUNCIONES MI ESPACIO Y MIS LECTURAS
--          (migration_nuevas_funciones.sql)
-- ============================================================
...
-- ============================================================
-- PARTE 3: CARPETAS PERSONALIZADAS EN MI ESPACIO
--          (migration_carpetas.sql)
-- ============================================================
...
-- ============================================================
-- PARTE 4: PREVIEW PDF PRE-GENERADO PARA ESTUDIOS PPTX
--          (migration_preview_pdf.sql)
-- ============================================================
...
-- ============================================================
-- PARTE 5: MÓDULO IA — CACHÉ DE RESÚMENES Y CONTROL DE USO
-- Fuente: ai_tables.sql
-- ============================================================

-- ============================================================
-- TABLA: ai_resumen_cache
-- Caché unificado de resúmenes generados por IA (ESTUDIO y NORMATIVA)
-- ============================================================
...
-- ============================================================
-- TABLA: ai_usage_log
-- Registro de uso de IA por usuario para rate limiting diario
-- ============================================================
...
-- FIN DEL SCRIPT
-- Total tablas: 33
--   Base (28): rol, usuarios, usuario_empresa, identificacion,
--     solicitud_registro, intento_login, recuperacion_contrasena,
--     registro_sesiones, categoria, estudios, estudio_categoria,
--     normativas, normativa_categoria, registro_descarga,
--     notificaciones, indicador, indicador_valor, publicacion,
--     actividad_sistema, auditoria_acciones_admin,
--     historial_estudios, historial_normativas, historial_roles,
--     dominio_autorizado, politica_contrasena,
--     configuracion_chatbot, chat_sesion, chat_mensaje
--   Nuevas (+5): contenido_guardado, historial_lectura,
--     espacio_carpeta, ai_resumen_cache, ai_usage_log
-- ============================================================
